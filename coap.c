...
static coap_endpoint_t endpoints[MAX_ENDPOINTS];
static int next_endpoint = 0;
...
int coap_parseToken(coap_buffer_t *tokbuf, const coap_header_t *hdr,
...
int coap_parseOptionsAndPayload(coap_option_t *options, uint8_t *numOptions,
		coap_buffer_t *payload, const coap_header_t *hdr, const uint8_t *buf,
		size_t buflen) {
	...
	while ((optionIndex < *numOptions) && (p < end) && (*p != 0xFF)) {
		if (0
				!= (rc = coap_parseOption(&options[optionIndex], &delta, &p,
						end - p)))
			return rc;
		optionIndex++;
	}
...
//    coap_dumpHeader(&hdr);
	...
	pkt->numopts = MAXOPT;
	if (0
			!= (rc = coap_parseOptionsAndPayload(pkt->opts, &(pkt->numopts),
...
const coap_option_t *coap_findOptions(const coap_packet_t *pkt, uint8_t num,
		uint8_t *count) {
...
static int handle_get_well_known_core(coap_rw_buffer_t *scratch,
		const coap_packet_t *inpkt, coap_packet_t *outpkt, uint8_t id_hi,
		uint8_t id_lo) {
	return coap_make_response(scratch, outpkt, (const uint8_t *) rsp,
			strlen(rsp), id_hi, id_lo, &inpkt->tok, COAP_RSPCODE_CONTENT,
...
int add_endpoint(coap_endpoint_t *new_endpoint) {
	if (next_endpoint == MAX_ENDPOINTS) {
		return COAP_ERR_BUFFER_TOO_SMALL;
	}

	endpoints[next_endpoint++] = *new_endpoint;
	build_rsp();
	return COAP_ERR_NONE;
}

void coap_setup(void) {
	coap_endpoint_t well_known_core_endpoint = { COAP_METHOD_GET,
			handle_get_well_known_core, &path_well_known_core, "ct=40" };
	add_endpoint(&well_known_core_endpoint);
}
